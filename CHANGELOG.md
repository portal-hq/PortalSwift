...
- Security: the provider attaches the Portal credential to an RPC request based on the URL's host (`isPortalOwnedUrl`), not a string prefix. `https://api.portalhq.io.attacker.com` no longer receives the credential; `web.portalhq.io`, an uppercase spelling and — as a consequence — `localhost` / `127.0.0.1` RPC URLs supplied through `withRpcConfig` now do (Android parity).
...
- Changed: hosts an SDK instance is configured with (`apiHost`, `mpcHost`, `enclaveMPCHost`, PortalConnect's `webSocketServer`, `PortalAuth`'s `apiHost`) are treated as Portal-owned for the 401 and trace gates, and can be added explicitly through `PortalOwnedHosts.register(_:)`. Configured and registered hosts match by exact hostname — only Portal's own `portalhq.io` / `portalhq.dev` apexes also cover their subdomains, as in the Web SDK — so a second custom host has to be configured or registered on its own. Previously a custom host outside `portalhq.io` silently disabled session invalidation. The RPC bearer is scoped tighter: `PortalProvider` attaches the credential only to the static Portal hosts and to the hosts its *own* `Portal` was configured with (`configuredHosts:`), never to a host another instance registered, so one instance's credential cannot reach another's proxy through an `rpcConfig` URL. RPC URLs from `withRpcConfig` are never registered.
...
- Changed: `FirebaseStorage` attributes a second 401 to the Portal credential only when the Firebase ID token actually changed on refresh. When the host's `getToken` callback returns the same token, the request fails with the new `FirebaseStorageError.tokenNotRefreshed` instead of signing the user out.
