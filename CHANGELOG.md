...
- Added `GDriveStorage.signOut()` to clear the stored Google session (parity with Android's `GoogleStorage.signOut()`). Throws `GDriveClientError.authenticationNotInitialized` if Google Drive has not been configured.
