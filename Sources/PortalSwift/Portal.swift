...
public final class Portal: PortalProtocol {
  ...
  ///
  /// - Parameters:
  ///   - chainId: The CAIP-2 chain identifier for the Bitcoin network (e.g., "bip122:000000000019d6689c085ae165831e93-p2wpkh" for Bitcoin Mainnet or "bip122:000000000933ea01ad0ee984209779ba-p2wpkh" for Bitcoin Testnet).
  ///   - params: The transaction parameters, encapsulated in a `BuildTransactionParam` struct, which includes:
  ///     - `to`: The recipient's Bitcoin address (must be a valid P2WPKH address, e.g., starting with `bc1q` or `tb1q`).
  ///     - `token`: The token to send, set to `"NATIVE"` for BTC (other tokens are not supported for Bitcoin P2WPKH).
  ///     - `amount`: The amount to send as a string, in BTC (e.g., `"0.001"` for 0.001 BTC).
  ///
  /// - Returns: A `BuildBitcoinP2wpkhTransactionResponse` containing:
  ///   - `transaction`: An object with:
  ///     - `rawTxHex`: The raw transaction hexadecimal string representing the PSBT.
  ///     - `signatureHashes`: An array of hexadecimal strings, each representing a hash that must be signed using `rawSign`.
  ///   - `metadata`: Additional transaction metadata.
  ///   - `error`: Error information if the transaction construction fails (nil on success).
  ///
  /// - Throws: API-related errors if the transaction construction fails (e.g., insufficient funds or network issues).
  ///
  /// - Note:
  ///   - The wallet must have a valid Bitcoin address with completed signing shares for the specified `chainId`. Use `doesWalletExist(_:)` to verify wallet existence.
  ///   - The `amount` does not include transaction fees, which are automatically calculated and deducted from the sender’s balance.
  ///   - After constructing the transaction, each `signatureHash` in the response must be signed using `rawSign` in the same order to produce valid signatures.
  ...
  /// - Parameters:
  ///   - chainId: The CAIP-2 chain identifier for the Bitcoin network (e.g., "bip122:000000000019d6689c085ae165831e93-p2wpkh" for Bitcoin Mainnet or "bip122:000000000933ea01ad0ee984209779ba-p2wpkh" for Bitcoin Testnet).
  ///   - params: The parameters required to broadcast the transaction, encapsulated in a `BroadcastParam` struct. It includes:
  ///     - `signatures`: An array of raw signatures corresponding to the signature hashes generated by `buildBitcoinP2wpkhTransaction`. Each signature is a hexadecimal string.
  ///     - `rawTxHex`: The raw transaction hexadecimal string (PSBT) generated by `buildBitcoinP2wpkhTransaction`.
  ///
  /// - Returns: A `BroadcastBitcoinP2wpkhTransactionResponse` containing:
  ///   - `data`: An object with the transaction hash (`txHash`) and other metadata (e.g., explorer URL if available).
  ///   - `metadata`: Additional transaction metadata.
  ///   - `error`: Error information if the broadcast fails (nil on success).
  ///
  /// - Throws: API-related errors if the broadcast request fails (e.g., network issues or invalid signatures).
  ///
  /// - Note:
  ///   - Ensure the `signatures` array matches the order and number of signature hashes returned by `buildBitcoinP2wpkhTransaction`.
  ///   - The `rawTxHex` must be the exact PSBT returned by `buildBitcoinP2wpkhTransaction` and must not be modified.
  ...
  public func sendAsset(chainId: String, params: SendAssetParams) async throws -> SendAssetResponse {
    ...
    case .bip122:
      ...
      // Construct and return response
      return SendAssetResponse(txHash: broadcastResponse.data.txHash)
  ...
  private static func buildDefaultRpcConfig(_ apiHost: String) -> [String: String] {
    [
      ...
      "eip155:10143" : "https://api.portalhq.io/rpc/v1/eip155/10143", // Monad Testnet
      "eip155:143" : "https://api.portalhq.io/rpc/v1/eip155/143" // Monad Mainnet
    ]
  }
}
