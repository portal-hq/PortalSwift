...
/// through `resolveCredentialToken(_:)`, which is what lets a session rotate or be
...
/// monitor behind `invalidateCredentials(_:)`, the once-only "reported" guard behind
/// `reportUnauthorized(_:)` and the host's `onSessionInvalidated` subscriptions) is
...
public protocol PortalCredentials: AnyObject {
  ...
  /// and must not perform I/O. Throw `PortalCredentialError.sessionInvalidated` once the
...
/// `reportUnauthorized(_:)`), and a listener registered for it can never fire.
public final class StaticCredentials: PortalCredentials {
  /// The raw key, exposed so `staticApiKeyOf(_:)` can bridge the subsystems that still
  ...
  /// `resolveCredentials(apiKey:credentials:)` and `resolveCredentialToken(_:)`, not
