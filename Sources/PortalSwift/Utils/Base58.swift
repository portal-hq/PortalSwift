...
public enum Base58 {
  ...
  public static func base58CheckDecode(_ input: String) -> [UInt8]? {
    ...
    let decodedChecksum = decodedChecksummedBytes.suffix(self.checksumLength)
    let decodedBytes = decodedChecksummedBytes.prefix(upTo: decodedChecksummedBytes.count - self.checksumLength)
    let calculatedChecksum = self.calculateChecksum([UInt8](decodedBytes))
  ...
  public static func base58Encode(_ bytes: [UInt8]) -> String {
    ...
    while integerBytes > 0 {
      ...
      answer.insert(self.alphabet[Int(remainder)], at: 0)
      integerBytes = quotient
    }

    let prefix = Array(bytes.prefix { $0 == 0 }).map { _ in self.alphabet[0] }
  ...
  public static func base58Decode(_ input: String) -> [UInt8]? {
    ...
    return Array(byteString.prefix { i in i == self.alphabet[0] }) + bytes
  }
  ...
  private static func calculateChecksum(_ input: [UInt8]) -> [UInt8] {
    let hashedData = self.sha256(input)
    let doubleHashedData = self.sha256(hashedData)
    let doubleHashedArray = Array(doubleHashedData)
    return Array(doubleHashedArray.prefix(self.checksumLength))
  }
