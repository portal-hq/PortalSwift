...
public final class KeychainStore: NSObject, AuthSessionStore {
  ...
  public func save(authSession: AuthSession) throws {
    try self.save(authSession: authSession, withItemName: self.itemName)
  }
  ...
  public func save(authSession: AuthSession, withItemName itemName: String) throws {
    ...
    try self.keychainHelper.setPassword(
  ...
  private func authSessionData(
    ...
    self.lastUsedKeyedArchiver = keyedArchiver
  ...
  @objc public func removeAuthSession(withItemName itemName: String) throws {
    try self.keychainHelper.removePassword(forService: itemName)
  }

  @objc public func removeAuthSession() throws {
    try self.keychainHelper.removePassword(forService: self.itemName)
  }
  ...
  private func keyedUnarchiver(forData data: Data) throws -> NSKeyedUnarchiver {
    ...
    self.lastUsedKeyedUnarchiver = keyedUnarchiver
  ...
  @objc public func retrieveAuthSession() throws -> AuthSession {
    let passwordData = try keychainHelper.passwordData(forService: self.itemName)
    ...
    guard let auth = keyedUnarchiver.decodeObject(
      ...
      throw Error.failedToConvertKeychainDataToAuthSession(itemName: self.itemName)
    }
    return auth
  }
  ...
  @objc public func retrieveAuthSessionInGTMOAuth2Format(
    ...
    let password = try keychainHelper.password(forService: self.itemName)
  ...
  @objc public func retrieveAuthSessionForGoogleInGTMOAuth2Format(
    ...
    return try self.retrieveAuthSessionInGTMOAuth2Format(
  ...
  @objc public func saveWithGTMOAuth2Format(
    ...
    guard let persistence = GTMOAuth2Compatibility
      .persistenceResponseString(forAuthSession: authSession)
    else {
...
public extension KeychainStore {
  ...
  enum Error: Swift.Error, Equatable, CustomNSError {
    ...
    public var errorUserInfo: [String: Any] {
      switch self {
      case let .unhandled(status: status):
        return ["status": status]
      case let .passwordNotFound(itemName):
        return ["itemName": itemName]
      case .noService:
        return [:]
      case let .unexpectedPasswordData(itemName):
        return ["itemName": itemName]
      case let .failedToCreateResponseStringFromAuthSession(authSession):
        return ["authSession": authSession]
      case let .failedToConvertRedirectURItoURL(redirectURI):
        return ["redirectURI": redirectURI]
      case .failedToConvertAuthSessionToData:
        return [:]
      case let .failedToConvertKeychainDataToAuthSession(itemName: itemName):
        return ["itemName": itemName]
      case let .failedToDeletePassword(itemName):
        return ["itemName": itemName]
      case let .failedToDeletePasswordBecauseItemNotFound(itemName: itemName):
        return ["itemName": itemName]
      case let .failedToSetPassword(forItemName: itemName):
        return ["itemName": itemName]
      }
    }
