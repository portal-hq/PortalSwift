...
public final class AuthSession: NSObject, GTMSessionFetcherAuthorizer, NSSecureCoding {
  ...
  public convenience init(authState: OIDAuthState) {
    self.init(
      authState: authState,
      serviceProvider: nil,
      userID: nil,
  ...
  @objc public func encode(with coder: NSCoder) {
    coder.encode(self.authState, forKey: AuthSession.authStateKey)
    coder.encode(self.serviceProvider, forKey: AuthSession.serviceProviderKey)
    coder.encode(self.userID, forKey: AuthSession.userIDKey)
    coder.encode(self.userEmail, forKey: AuthSession.userEmailKey)
    coder.encode(self._userEmailIsVerified, forKey: AuthSession.userEmailIsVerifiedKey)
  }
  ...
  public func authorizeRequest(
    ...
    self.authorizeRequest(withArguments: arguments)
  }
  ...
  private func authorizeRequest(withArguments args: AuthorizationArguments) {
    self.serialAuthArgsQueue.sync {
      self.authorizationArgs.append(args)
    }
    let additionalRefreshParameters = self.delegate?.additionalTokenRefreshParameters?(
      forAuthSession: self
    )
    let authStateAction = {
      (accessToken: String?, _: String?, error: Swift.Error?) in
  ...
  private func authorizeRequestImmediately(args: AuthorizationArguments, accessToken: String?) {
    ...
    let callbackQueue = self.fetcherService?.callbackQueue ?? DispatchQueue.main

    let callback: () -> Void = {
      callbackQueue.async {
        switch args.callbackStyle {
        case let .completion(callback):
          self.invokeCompletionCallback(with: callback, error: args.error)
        case let .delegate(delegate, selector):
          self.invokeCallback(
  ...
  @objc public func stopAuthorization(for request: URLRequest) {
    self.serialAuthArgsQueue.sync {
      ...
      self.authorizationArgs.remove(at: index)
    }
  }
  ...
  @objc public func isAuthorizingRequest(_ request: URLRequest) -> Bool {
    ...
    self.serialAuthArgsQueue.sync {
      argsWithMatchingRequest = self.authorizationArgs.first { args in
  ...
  @objc public var canAuthorize: Bool {
    return self.authState.isAuthorized
  }
  ...
  @objc public func primeForRefresh() -> Bool {
    guard self.authState.refreshToken == nil else {
      return false
    }
...
public extension AuthSession {
  ...
  enum Error: Swift.Error, Equatable, CustomNSError {
    ...
    public var errorUserInfo: [String: Any] {
      switch self {
      case let .cannotAuthorizeRequest(request):
        return ["request": request]
      case let .accessTokenEmptyForRequest(request):
        return ["request": request]
      }
    }
