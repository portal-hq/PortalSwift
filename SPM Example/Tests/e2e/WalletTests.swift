...
final class WalletTests: XCTestCase {
  ...
  func testLogin() async throws {
    ...
    let user = user != nil
      ? try await self.viewController.signUp(self.username)
      : try await self.viewController.signIn(self.username)
    self.user = user
    XCTAssertNotNil(user.exchangeUserId)
    XCTAssertNotNil(user.clientApiKey)
    expectation.fulfill()
...
enum WalletTestsError: Error {
  case couldNotFindUIElement(String)
}
