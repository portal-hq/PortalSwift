...
public class WSCompression: CompressionHandler {
  ...
  public func load(headers: [String: String]) {
    ...
    self.decompressorTakeOver = false
    self.compressorTakeOver = false

    // assume defaults unless the headers say otherwise
    self.compressor = Compressor(windowBits: 15)
    self.decompressor = Decompressor(windowBits: 15)
    ...
    for p in parts {
      ...
      if part.hasPrefix("server_max_window_bits=") {
        ...
        if let val = Int(valString.trimmingCharacters(in: .whitespaces)) {
          self.decompressor = Decompressor(windowBits: val)
        }
      } else if part.hasPrefix("client_max_window_bits=") {
        let valString = part.components(separatedBy: "=")[1]
        if let val = Int(valString.trimmingCharacters(in: .whitespaces)) {
          self.compressor = Compressor(windowBits: val)
        }
      } else if part == "client_no_context_takeover" {
        self.compressorTakeOver = true
      } else if part == "server_no_context_takeover" {
        self.decompressorTakeOver = true
      }
    }
  }
...
class Decompressor {
  ...
  init?(windowBits: Int) {
    ...
    guard self.initInflate() else { return nil }
  }

  private func initInflate() -> Bool {
    if Z_OK == inflateInit2_(&self.strm, -CInt(self.windowBits),
                             ZLIB_VERSION, CInt(MemoryLayout<z_stream>.size))
    {
      self.inflateInitialized = true
      return true
    }
    return false
  }
  ...
  func reset() throws {
    self.teardownInflate()
    guard self.initInflate() else { throw WSError(type: .compressionError, message: "Error for decompressor on reset", code: 0) }
  }

  func decompress(_ data: Data, finish: Bool) throws -> Data {
    return try data.withUnsafeBytes { (bytes: UnsafePointer<UInt8>) -> Data in
      try self.decompress(bytes: bytes, count: data.count, finish: finish)
    }
  }
  ...
  private func decompress(bytes: UnsafePointer<UInt8>, count: Int, out: inout Data) throws {
    var res: CInt = 0
    self.strm.next_in = UnsafeMutablePointer<UInt8>(mutating: bytes)
    self.strm.avail_in = CUnsignedInt(count)

    repeat {
      self.buffer.withUnsafeMutableBytes { bufferPtr in
        self.strm.next_out = bufferPtr.bindMemory(to: UInt8.self).baseAddress
        self.strm.avail_out = CUnsignedInt(bufferPtr.count)

        res = inflate(&self.strm, 0)
      }

      let byteCount = self.buffer.count - Int(self.strm.avail_out)
      out.append(self.buffer, count: byteCount)
    } while res == Z_OK && self.strm.avail_out == 0

    guard (res == Z_OK && self.strm.avail_out > 0)
      || (res == Z_BUF_ERROR && Int(self.strm.avail_out) == self.buffer.count)
    else {
  ...
  private func teardownInflate() {
    if self.inflateInitialized, Z_OK == inflateEnd(&self.strm) {
      self.inflateInitialized = false
    }
  }
...
class Compressor {
  ...
  init?(windowBits: Int) {
    self.windowBits = windowBits
    guard self.initDeflate() else { return nil }
  }

  private func initDeflate() -> Bool {
    if Z_OK == deflateInit2_(&self.strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED,
                             -CInt(self.windowBits), 8, Z_DEFAULT_STRATEGY,
                             ZLIB_VERSION, CInt(MemoryLayout<z_stream>.size))
    {
      self.deflateInitialized = true
      return true
    }
    return false
  }
  ...
  func reset() throws {
    self.teardownDeflate()
    guard self.initDeflate() else { throw WSError(type: .compressionError, message: "Error for compressor on reset", code: 0) }
  }
  ...
  func compress(_ data: Data) throws -> Data {
    ...
    data.withUnsafeBytes { (ptr: UnsafePointer<UInt8>) in
      self.strm.next_in = UnsafeMutablePointer<UInt8>(mutating: ptr)
      self.strm.avail_in = CUnsignedInt(data.count)
      ...
      repeat {
        self.buffer.withUnsafeMutableBytes { bufferPtr in
          ...
          res = deflate(&self.strm, Z_SYNC_FLUSH)
        }

        let byteCount = self.buffer.count - Int(self.strm.avail_out)
        compressed.append(self.buffer, count: byteCount)
      } while res == Z_OK && self.strm.avail_out == 0
    }
  ...
  private func teardownDeflate() {
    if self.deflateInitialized, Z_OK == deflateEnd(&self.strm) {
      self.deflateInitialized = false
    }
  }
