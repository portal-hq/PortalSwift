...
public class WSFramer: Framer {
  ...
  public func updateCompression(supports: Bool) {
    self.compressionEnabled = supports
  }
  ...
  private func process() -> ProcessEvent {
    ...
    var pointer = [UInt8]()
    self.buffer.withUnsafeBytes { pointer.append(contentsOf: $0) }
    ...
    if self.compressionEnabled && opcode != .continueFrame {
      needsDecompression = (RSV1Mask & pointer[0]) > 0
    }
    if !self.isServer && (isMasked > 0 || RSV1 > 0) && opcode != .pong && !needsDecompression {
