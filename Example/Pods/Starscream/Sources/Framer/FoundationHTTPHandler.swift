...
import Foundation
  ...
  public class FoundationHTTPHandler: HTTPHandler {
    ...
    public func parse(data: Data) -> Int {
      let offset = self.findEndOfHTTP(data: data)
      if offset > 0 {
        self.buffer.append(data.subdata(in: 0 ..< offset))
      } else {
        self.buffer.append(data)
      }
      if self.parseContent(data: self.buffer) {
        self.buffer = Data()
      }
      return offset
    }
    ...
    func parseContent(data: Data) -> Bool {
      ...
      if let cfHeaders = CFHTTPMessageCopyAllHeaderFields(response) {
        ...
        if code != HTTPWSHeader.switchProtocolCode {
          self.delegate?.didReceiveHTTP(event: .failure(HTTPUpgradeError.notAnUpgrade(code, headers)))
          return true
        }

        self.delegate?.didReceiveHTTP(event: .success(headers))
        return true
      }

      self.delegate?.didReceiveHTTP(event: .failure(HTTPUpgradeError.invalidData))
      return true
    }
